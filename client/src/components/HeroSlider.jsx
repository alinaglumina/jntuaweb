      ...
      <div className={`container relative grid ${height} place-items-center py-16`}>
        <AnimatePresence mode="wait">
          <motion.div key={s._id || i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="max-w-2xl text-center">
            ...
            <h2 className="font-display text-3xl font-bold text-white md:text-5xl">{s.title}</h2>
            {s.subtext && <p className="mt-3 text-white/80">{s.subtext}</p>}
          </motion.div>
        </AnimatePresence>
      </div>
      {items.length > 1 && (
        <>
          <button onClick={() => go(-1)} aria-label="Previous" className="absolute left-3 top-1/2 -translate-y-1/2 rounded-full bg-white/10 px-3 py-2 hover:bg-white/20">‹</button>
          <button onClick={() => go(1)} aria-label="Next" className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full bg-white/10 px-3 py-2 hover:bg-white/20">›</button>
          <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-2">
            {items.map((_, k) => <button key={k} onClick={() => setI(k)} aria-label={`Slide ${k + 1}`} className={`h-2 w-2 rounded-full ${k === i % items.length ? 'bg-gold' : 'bg-white/40'}`} />)}
          </div>
        </>
      )}
    </section>
