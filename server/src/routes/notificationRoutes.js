...
const c = crudController(Notification, {
  ...
  baseFilter: (req) => ({ ...(req.user ? {} : { isActive: true }), ...(req.query.category ? { category: { $in: [req.query.category] } } : {}), ...(req.query.directorateKey ? { directorateKey: req.query.directorateKey } : {}) }),
});
